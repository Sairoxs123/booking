...
def booking():
    form = UploadForm()
    return render_template("desktop/booking.html", form = form)
...
def check():
    if request.method == "POST":
        ...
        return render_template("desktop/available.html", info = results, date = date, start = starting_periods, end = ending_periods, form = form)
...
def delbook():
    ...
    form = UploadForm()
    return render_template("desktop/delete.html", start = starting_periods, end = ending_periods, form = form)
...
def admin_login():
    ...
    if request.method == "POST":
        ...
        if not login:
            return render_template("desktop/admin-login.html", error = "empty", form = form)

        if login != admin_password:
            return render_template("desktop/admin-login.html", error = "wrong", form = form)

        session["logged-in"] = True

        return redirect("/desktop/admin-access")

    return render_template("desktop/admin-login.html", form = form)
...
def access():
    if session.get("logged-in"):
        ...
        return render_template("desktop/admin.html", results = results, form = form)
...
def mobile_booking():
    form = UploadForm()
    return render_template("mobile/booking.html", form = form)
